// Simple integration example - just add NFT minting to existing MatchFrame
...
interface SimpleMatchFrameProps {
  matchedProfile: CharmProfile;
  onContinue: () => void;
}

export default function SimpleMatchFrame({ 
  matchedProfile, 
  onContinue 
}: SimpleMatchFrameProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-400 via-purple-500 to-indigo-600 flex items-center justify-center p-4">
      ...
      >
        ...
        <motion.div
          ...
          className="space-y-4"
        >
          {/* Show NFT Minting Option */}
          {!showNFTMinting && address && (
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => setShowNFTMinting(true)}
              className="w-full py-3 px-6 bg-gradient-to-r from-yellow-400 to-orange-500 text-white font-bold rounded-full shadow-xl hover:shadow-2xl transition-all duration-300"
            >
              🏆 Mint Match NFT on Celo
            </motion.button>
          )}
        ...
        >
          ...
          {address && <p>🎨 Mint an NFT to commemorate this match!</p>}
        </motion.div>
      </motion.div>
    </div>
  );
